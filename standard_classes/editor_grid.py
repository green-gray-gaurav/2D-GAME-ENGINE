...
class editorGrid():
    ...
    def renderComponent(self):
        ...
        if self.visible:
        # horizontal
            pygame.draw.line(self.window , self.color2 , (self.x - self.screenDim[0] , self.y) , (self.x + self.screenDim[0] , self.y) , self.width)
        #veriacal line
            pygame.draw.line(self.window , self.color2 , (self.x  , self.y - self.screenDim[1]) , (self.x  , self.y + self.screenDim[1]) , self.width)
        ...
        if self.Rect != None:
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if self.Rect.collidepoint(event.pos):
                    ...
                    self.displacePointer=[self.x -mx , self.y - my]
    ...
    def render(self  , objArray):
        ...
        if self.selected:
            grid , scn = objArray
                 ...
                 #here ew are doing the acl for displacement vector
            mx , my = pygame.mouse.get_pos()
            dx = self.x - mx
            dy = self.y - my
            ...
            #here to make the snappin system
                ...
                self.x ,self.y  = mx + self.displacePointer[0] , my + self.displacePointer[1]
